...
def APPLY_JOB():
    ...
    result = Applied_EMP.insert_one({"job_id":ObjectId(job_id),"user_id":ObjectId(session['user_id']),"User_name":session['user_name'],"Matching_percentage":match_percentage, "Resume_name": session["resume_name"]})
    if result == None:
...
def view_applied_candidates():
    job_id = request.form['job_id']
    result_data = None
    result_data = Applied_EMP.find({"job_id":ObjectId(job_id)},{"User_name":1,"Matching_percentage":1, "Resume_name": 1,"user_id": 1}).sort([("Matching_percentage",-1)])
    if result_data == None:
        ...
        for i in result_data:
            result[cnt+2] = {"Name":i['User_name'],"Match":i['Matching_percentage'], "Resume_name": i["Resume_name"]}
            cnt+=1   
        result[0]=cnt
